...
async function loadCountry(code) {
  ...
  try {
    ...
    const rateResp = await fetch(
      `https://api.exchangerate.host/latest?base=${countryData.currencyCode}&symbols=USD`
    );
...
async function showWeather() {
  ...
  try {
    const [cur, fc] = await Promise.all([
      fetch(
        `https://api.openweathermap.org/data/2.5/weather?lat=${countryData.lat}&lon=${countryData.lon}&units=metric&appid=${weatherAPIKey}`
      ).then((r) => r.json()),
      fetch(
        `https://api.openweathermap.org/data/2.5/forecast?lat=${countryData.lat}&lon=${countryData.lon}&units=metric&appid=${weatherAPIKey}`
      ).then((r) => r.json()),
    ]);
  ...
  navigator.geolocation.getCurrentPosition(
    function (pos) {
      $.getJSON(
        ...
        .fail(() => alert("Unable to fetch weather."))
        .always(hideLoader);
    },
    function () {
      hideLoader();
...
$(function () {
  ...
  $("#btnName").click(() => {
    $("#bodyName").html(
      `<strong>${countryData.name}</strong><br>Capital: ${countryData.capital}`
    );
  ...
  $("#btnCurrency").click(() => {
    $("#bodyCurrency").html(
      `Currency: ${countryData.currencyName} (${
        countryData.currencyCode
      })<br>1 ${countryData.currencyCode} ≈ ${
        countryData.exchangeRate?.toFixed(4) || "N/A"
      } USD`
    );
  ...
  $("#btnWiki").click(() => {
    const u = `https://en.wikipedia.org/wiki/${encodeURIComponent(
      countryData.name
    )}`;
    $("#bodyWiki").html(
      `<a href="${u}" target="_blank">${countryData.name} on Wikipedia</a>`
    );
